...
import com.funtap.awass.rabbitmq.configuration.PHPEvalFunctionConfiguration;
...
@Configuration
public class PHPEvalFunctionListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = PHPEvalFunctionConfiguration.QUEUE_PHPeva,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            PHP_Eval_Function phpev = new PHP_Eval_Function();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            String cookie = null;
            boolean check = phpev.PHP_Eval_Funtion(uri,cookie);
...
//done sub to
            ...
            if(check) {
                ...
                reportScan.setResult("PHP Eval Function");
