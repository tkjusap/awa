...
@Configuration
public class PHPinjectionListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = PHPinjectionConfiguration.QUEUE_PHPi,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            PHPInjection phpi = new PHPInjection();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            boolean check = phpi.Scan(uri);
...
//            upjob.updatejob(uri.getIdTarget(),-1);
            ...
            if(check) {
                ...
                reportScan.setResult("PHP Injection");
