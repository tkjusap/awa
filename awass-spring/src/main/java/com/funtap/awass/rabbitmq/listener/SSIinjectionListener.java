...
import com.funtap.awass.Top10OWASPWeb2021.Scan.A3Injection.PHPInjection;
import com.funtap.awass.Top10OWASPWeb2021.Scan.A3Injection.SSIinjection;
...
import com.funtap.awass.rabbitmq.configuration.PHPinjectionConfiguration;
import com.funtap.awass.rabbitmq.configuration.SSIinjectionConfiguration;
...
@Configuration
public class SSIinjectionListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = SSIinjectionConfiguration.QUEUE_SSIi,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            SSIinjection ssii = new SSIinjection();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            boolean check = ssii.Scan(uri);
...
//            upjob.updatejob(uri.getIdTarget(),-1);
            ...
            if(check) {
                ...
                reportScan.setResult("Server-Side Includes (SSI) Injection");
