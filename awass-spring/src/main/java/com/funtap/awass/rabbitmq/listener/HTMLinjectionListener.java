...
import com.funtap.awass.Top10OWASPWeb2021.Scan.A3Injection.CMDinjection;
...
import com.funtap.awass.rabbitmq.configuration.CMDInjectionConfiguration;
...
@Configuration
public class HTMLinjectionListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = HMTLInjectionConfiguration.QUEUE_HTMLi,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            HTMLinjection htmli = new HTMLinjection();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            boolean check = htmli.Scan(uri);
...
//            upjob.updatejob(uri.getIdTarget(),-1);
            ...
            if(check) {
                ...
                reportScan.setResult("HTML Injection");
