...
import com.funtap.awass.Top10OWASPWeb2021.Scan.A7IdentificationAndAuthenticationFailures.PasswordAttacks;
...
import com.funtap.awass.rabbitmq.configuration.PasswordAttacksConfiguration;
...
public class PasswordAttacksListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = PasswordAttacksConfiguration.QUEUE_PasswordAttacks,concurrency = "5")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            PasswordAttacks PA  = new PasswordAttacks();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            String cookie = null;
            String urlx = uri.getUrl();
            String domain = uri.getUrl().split("/")[2].trim();
            boolean check = PA.PasswordAttacks(uri);
            ...
            if(check) {
                ...
                reportScan.setResult("Password Attacks");
