...
@Configuration
public class CMDInjectionListener {
    @Autowired
    Gson gson;

    @RabbitListener(queues = CMDInjectionConfiguration.QUEUE_CMDi,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            CMDinjection cmdi = new CMDinjection();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            boolean check = cmdi.Scan(uri);
...
//            //done sub to
            ...
            if(check) {
                ...
                reportScan.setResult("OS Command Injection");
