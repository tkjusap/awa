...
import com.funtap.awass.rabbitmq.configuration.InsecureLoginFormsConfiguration;
...
@Configuration
public class InsecureLoginFormsListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = InsecureLoginFormsConfiguration.QUEUE_IL,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            Insecure_Login_Forms il = new Insecure_Login_Forms();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            String cookie = null;
            boolean check = il.Insecure_Login_Forms(uri,cookie);
...
//done sub to
            ...
            if(check) {
                ...
                reportScan.setResult("Insecure Login Forms");
