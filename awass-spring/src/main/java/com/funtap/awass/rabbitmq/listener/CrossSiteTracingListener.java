...
import com.funtap.awass.Top10OWASPWeb2021.Scan.A5SecurityMisconfiguration.Cross_Site_Tracing;
...
import com.funtap.awass.rabbitmq.configuration.CrossSiteTracingConfiguration;
...
@Configuration
public class CrossSiteTracingListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = CrossSiteTracingConfiguration.QUEUE_XST,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            Cross_Site_Tracing xst = new Cross_Site_Tracing();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            String cookie = null;
            boolean check = xst.Cross_Site_Tracing(uri,cookie);
...
//done sub to
            ...
            if(check) {
                ...
                reportScan.setResult("Cross Site Tracking (XST)");
