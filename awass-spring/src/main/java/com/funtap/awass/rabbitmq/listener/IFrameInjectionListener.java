...
import com.funtap.awass.Top10OWASPWeb2021.Scan.A3Injection.HTMLinjection;
import com.funtap.awass.Top10OWASPWeb2021.Scan.A3Injection.IFrameinjection;
...
import com.funtap.awass.rabbitmq.configuration.HMTLInjectionConfiguration;
import com.funtap.awass.rabbitmq.configuration.IFrameInjectionConfiguration;
...
@Configuration
public class IFrameInjectionListener {
    @Autowired
    Gson gson;


    @RabbitListener(queues = IFrameInjectionConfiguration.QUEUE_IFi,concurrency = "3")
    public void helloShipping(Message message) {
        try {
            UrlOb uri = gson.fromJson(new String(message.getBody()),UrlOb.class);
            IFrameinjection ifin = new IFrameinjection();
            OkHttpClient client = HttpCommon.getInstance().getHttpClient().newBuilder()
                    .build();
            boolean check = ifin.Scan(uri);
...
//            upjob.updatejob(uri.getIdTarget(),-1);
            ...
            if(check) {
                ...
                reportScan.setResult("IFRAME Injection");
